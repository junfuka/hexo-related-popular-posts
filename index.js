...
module.exports = function (hexo) {
  ...
  if (hexo.config.popularPosts) {
    if (hexo.config.popularPosts.googleAnalyticsAPI) {
      ...
      if (process.env.GOOGLE_API_CLIENT_ID && process.env.GOOGLE_API_EMAIL && process.env.GOOGLE_API_PRIVATE_KEY && process.env.GOOGLE_ANALYTICS_VIEW_ID) {
        id = process.env.GOOGLE_API_CLIENT_ID
        email = process.env.GOOGLE_API_EMAIL
        key = process.env.GOOGLE_API_PRIVATE_KEY
        viewId = process.env.GOOGLE_ANALYTICS_VIEW_ID
      } else if (hexo.config.popularPosts.googleAnalyticsAPI.clientId && hexo.config.popularPosts.googleAnalyticsAPI.serviceEmail && hexo.config.popularPosts.googleAnalyticsAPI.key && hexo.config.popularPosts.googleAnalyticsAPI.viewId) {
  ...
  if (cache_path && fs.existsSync(cache_path)) {
    ...
    const gaDataStr = fs.readFileSync(cache_path)
    ...
    const isNgwUpdate = !gaData[0].ngwHash || gaData[0].ngwHash != ngwHash
