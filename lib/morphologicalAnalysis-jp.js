...
module.exports.getKeyword = (config, post, callbackFn) => {
  if (config.popularPosts && config.popularPosts.morphologicalAnalysis !== undefined) {
    ...
    const kuromoji_path1 = pathFn.normalize(__dirname + '/../node_modules/kuromoji/dict/')
    const kuromoji_path2 = pathFn.normalize(__dirname + '/../../kuromoji/dict/')
    const path_case1 = fs.existsSync(kuromoji_path1)
    const path_case2 = fs.existsSync(kuromoji_path2)

    if (config.popularPosts.morphologicalAnalysis && config.popularPosts.morphologicalAnalysis.negativeKeywordsList) {
      const nwPath = pathFn.join(process.env.PWD || process.cwd(), config.popularPosts.morphologicalAnalysis.negativeKeywordsList)

      if (fs.existsSync(nwPath)) {
        const negativeWordTmp = fs.readFileSync(nwPath, 'utf-8')
...
const wait_takenize = (post, callbackFn) => {
  if (_tokenizer == null) {
    setTimeout(() => {
...
const call_wait_takenize = (post, callbackFn) => {
  wait_takenize(post, callbackFn)
}


const takenize = (post, callbackFn) => {
  let i = 0
  let k = 0
  let keyword = []
  const resultTKR = _tokenizer.tokenize(post.title + '\n' + util.decord_unicode(util.replaceHTMLtoText(post.content)))
  ...
  const wordsLength = keyword.length
