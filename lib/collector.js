...
const postMetaCollector = (inArgs) => {
  const _this = inArgs[0]
  const post = inArgs[1]
  const internalLinks = inArgs[2]
  const keywords = inArgs[3]
  const wordsLength = inArgs[4]
  const isUpdatePermission = inArgs[5]
    ...
    if (post.published && isUpdatePermission) {
      ...
      for (let i = 0; i < _this.config.popularPosts.tmp.gaData.length; i++) {
        if (_this.config.popularPosts.tmp.gaData[i].path == post.path) {
          const gaData_tmp = _this.config.popularPosts.tmp.gaData
          ...
          gaData_tmp[i].date = typeof(post.date) === 'string' ? post.date : moment(post.date).format()
      ...
      if (!isMatched) {
        const gaData_tmp = _this.config.popularPosts.tmp.gaData
        const newGaData = util.gaDataModel({
          'updated': String(post.updated._i),
          'title': post.title,
          'path': post.path,
          'eyeCatchImage': eyeCatchImage,
          'excerpt': description,
          'date': typeof(post.date) === 'string' ? post.date : moment(post.date).format(),
          'pv': 0,
...
const morphologicalAnalysis = (inArgs) => {
  const _this = inArgs[0]
  const post = inArgs[1]
  const internalLinks = inArgs[2]
  const isUpdatePermission = inArgs[3]
...
const internalLinkAnalytics = (inArgs) => {
  const _this = inArgs[0]
  const post = inArgs[1]
  const isUpdatePermission = inArgs[2]

  return new Promise((resolve, reject) => {
    const ilinks = post.content.match(/\<a\s*?href\=\"(.*?)\".*?\>/g)
    const internalLinks = []

    if (ilinks) {
      for (let i = 0; i < ilinks.length; i++) {
        const ilink = ilinks[i].match(/href\=\"\/(.*?)\"/)
...
const checkUpdate = (inThis, inPost) => {
  const _this = inThis
  const post = inPost
  const gaData = _this.config.popularPosts.tmp.gaData
