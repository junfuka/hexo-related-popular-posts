...
module.exports.getList = (inOptions, inPost, inHexo) => {
  ...
  if (!config.popularPosts.tmp.cache_path && cachePosts.length && cachePosts.data[0].popularPost_tmp_gaData) {
    ...
    for (let i = 0; i < cachePosts.length; i++) {
      // PV update
      if (config.popularPosts.tmp.isGaUpdate) {
        for (k = 0; k < tmp_gaData.length; k++) {
          if (cachePosts.data[i].popularPost_tmp_gaData.path == tmp_gaData[k].path) {
            cachePosts.data[i].popularPost_tmp_gaData.pv = tmp_gaData[k].pv
            cachePosts.data[i].popularPost_tmp_gaData.totalPV = tmp_gaData[k].totalPV
            break
          }
        }
      }

      config.popularPosts.tmp.gaData.push(cachePosts.data[i].popularPost_tmp_gaData)
      if (cachePosts.data[i].popularPost_tmp_postPath) config.popularPosts.tmp.postPath.push(cachePosts.data[i].path)
    }
    tmp_gaData = null
  }
  // --------------------------------------------
  ...
  const weight_of_tag_relevancy = config.popularPosts.tmp.weight_of_tag_relevancy
  const weight_of_contents_relevancy = config.popularPosts.tmp.weight_of_contents_relevancy
    ...
    if (postsTag != undefined && postsTag != '') {
      for (i = 0; i < config.popularPosts.tmp.gaData.length; i++) {
        if (config.popularPosts.tmp.gaData[i].date != '') {
          const p_tags = config.popularPosts.tmp.gaData[i].tags
  ...
  if (Number(inOptions.PPMixingRate) != 1.0) {
            ...
            for (k = 0; k < postsGaData.keywords.length; k++) {
              for (n = 0; n < config.popularPosts.tmp.gaData[i].keywords.length; n++) {
                if (postsGaData.keywords[k].w == config.popularPosts.tmp.gaData[i].keywords[n].w && postsGaData.categories == config.popularPosts.tmp.gaData[i].categories) {
                  ...
                  matchKeyWord.push({
                    'w': config.popularPosts.tmp.gaData[i].keywords[n].w,
                    'f': config.popularPosts.tmp.gaData[i].keywords[n].f,
                  })
                }
              }
            }
    ...
    relatedPosts.sort((a, b) => {
      const degreeA = a.relatedDegree * weight_of_tag_relevancy + a.relatedDegree_ma * weight_of_contents_relevancy
      const degreeB = b.relatedDegree * weight_of_tag_relevancy + b.relatedDegree_ma * weight_of_contents_relevancy
  ...
  for (i = 0; i < relatedCnt; i++) {
    if (i < relatedPosts.length) {
      const elm = getElm(relatedPosts[i])
  ...
  while (addedPath.length < Number(inOptions.maxCount) && popularCnt > 0 && i < popularPosts.length) {
    const elm = getElm(popularPosts[i])
    if (elm) {
      retrunJson.push(elm)
      popularCnt--
    }
