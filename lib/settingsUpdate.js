...
module.exports.chkUpdate = (config) => {
  ...
  const settingsStr = JSON.stringify(config.popularPosts)
  const shash = hasha(settingsStr, {algorithm: 'md5'})
  return shash
}
...
module.exports.chkUpdate_ngw = (config) => {
  ...
  const shash = hasha(negativeWordStr, {algorithm: 'md5'})
  return shash
}

module.exports.getMD5 = (inStr) => {
  if (!inStr || inStr == '') return ''
  const shash = hasha(inStr, {algorithm: 'md5'})
  return shash
}
